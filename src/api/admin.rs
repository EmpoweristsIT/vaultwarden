...
async fn has_http_access() -> bool {
    let Ok(req) = make_http_request(Method::HEAD, "https://github.com/empowerists/vaultwarden") else {
        return false;
    };
...
async fn get_release_info(has_http_access: bool) -> (String, String, String) {
    ...
    if has_http_access {
        (
            ...
            match get_json_api::<GitCommit>("https://api.github.com/repos/empowerists/vaultwarden/commits/main").await {
                Ok(mut c) => {
                    c.sha.truncate(8);
                    c.sha
                }
            ...
            match get_json_api::<GitRelease>("https://api.github.com/repos/empowerists/bw_web_builds/releases/latest")
                .await
            {
